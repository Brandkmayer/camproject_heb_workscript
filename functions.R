...
counts.df <- function(cameradf) {
  cameradf.counts <- data.frame(species = c("horse", "cattle", "elk", "muledeer"), 
             freq = (c(sum(cameradf$horse), sum(cameradf$cow), sum(cameradf$elk), sum(cameradf$deer))))
}

#If there is a need to create a column with percentages real quick. 

count2pct<- function(cameradf) {
  cameradf.percents <- for(col in names(cameradf)[-1]) {
    cameradf[paste0(col, "_pct")] = cameradf[col] / sum(cameradf[col])
  }
}

#If you want to add a sequence to groups 
seq_species <- function(cameradfspecies) {
  cameradfspecies$group_sequence <- 1  
  for (i in 2:(length(cameradfspecies[,1]))) {  
