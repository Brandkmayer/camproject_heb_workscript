...
```
*****Incomplete function***** Just run as is inside of the function
```{r}
bhvr_freq_bysp <- function(behavior_df) {
...
# ___________________________________ Data.frame presentation ___________________________________________
  Bhr_freq_h_build <- data.frame(Site = "Black Canyon", Species = "Horses", Behavior = c("Forage", "Drinking", "Movement", "Bedding", "standing", "Unknown"), Value = (c(length(forage_h$group[!duplicated(forage_h$group)]), length(drink_h$group[!duplicated(drink_h$group)]),length(walk_run_h$group[!duplicated(walk_run_h$group)]),length(bed_h$group[!duplicated(bed_h$group)]),length(stand_h$group[!duplicated(stand_h$group)]),length(unknown_h$group[!duplicated(unknown_h$group)]))))
    Bhr_freq_c_build <- data.frame(Site = "Black Canyon", Species = "Cattle", Behavior = c("Forage", "Drinking", "Movement", "Bedding", "standing", "Unknown"), Value = (c(length(forage_c$group[!duplicated(forage_c$group)]), length(drink_c$group[!duplicated(drink_c$group)]),length(walk_run_c$group[!duplicated(walk_run_c$group)]),length(bed_c$group[!duplicated(bed_c$group)]),length(stand_c$group[!duplicated(stand_c$group)]),length(unknown_c$group[!duplicated(unknown_c$group)]))))
    Bhr_freq_e_build <- data.frame(Site = "Black Canyon", Species = "Elk", Behavior = c("Forage", "Drinking", "Movement", "Bedding", "standing", "Unknown"), Value = (c(length(forage_e$group[!duplicated(forage_e$group)]), length(drink_e$group[!duplicated(drink_e$group)]),length(walk_run_e$group[!duplicated(walk_run_e$group)]),length(bed_e$group[!duplicated(bed_e$group)]),length(stand_e$group[!duplicated(stand_e$group)]),length(unknown_e$group[!duplicated(unknown_e$group)]))))
    Bhr_freq_d_build <- data.frame(Site = "Black Canyon", Species = "Muledeer", Behavior = c("Forage", "Drinking", "Movement", "Bedding", "standing", "Unknown"), Value = (c(length(forage_d$group[!duplicated(forage_d$group)]), length(drink_d$group[!duplicated(drink_d$group)]),length(walk_run_d$group[!duplicated(walk_run_d$group)]),length(bed_d$group[!duplicated(bed_d$group)]),length(stand_d$group[!duplicated(stand_d$group)]),length(unknown_d$group[!duplicated(unknown_d$group)]))))
bhr_freq_df_1 <- rbind(Bhr_freq_h_build, Bhr_freq_c_build, Bhr_freq_e_build, Bhr_freq_d_build)
bhr_freq_df_2 <- rbind(Bhr_freq_h_build, Bhr_freq_c_build, Bhr_freq_e_build, Bhr_freq_d_build)

bhr_freq_df_final <- rbind(bhr_freq_df_1, bhr_freq_df_2)
...
bhr_freq_df$Value[is.nan(bhr_freq_df$Value)] <- 0

barchart4sp_bhvfreq(bhr_freq_df, bhr_freq_df, "Black Canyon", "Black Canyon")
